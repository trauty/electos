...
export default async function Products({ params }: { params: { id: string } }) {
    const res = await fetch(process.env.BASE_URL + `/api/products?catId=${params.id}`);
    ...
    return (
        <div className="w-screen min-h-screen pt-20 p-10">
            ...
            <div className="h-full w-full rounded-xl px-4 py-2 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 2xl:grid-cols-7 lg:grid-cols-4 md:grid-cols-3 
